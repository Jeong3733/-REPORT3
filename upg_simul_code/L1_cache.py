...
class L1Cache:
    ...
    def read(self, address):
        ...
        if line:
            if (self._replace_pol == L1Cache.LRU or
                self._replace_pol == L1Cache.LFU):
    ...
    def load(self, address, data):
        ...
        tag = self._get_tag(address)  # Tag of cache line from L2
        set = self._get_set(address)  # Set of cache lines from L2
        victim_info = None

        # Select the victim
        if (self._replace_pol == L1Cache.LRU or
            self._replace_pol == L1Cache.LFU or
            self._replace_pol == L1Cache.FIFO):
            victim = set[0]
            ...
            if self._replace_pol == L1Cache.FIFO:
                self._update_use(victim, set)
        elif self._replace_pol == L1Cache.RAND:
            ...
            if line.use < self._mapping_pol:
                ...
                for other in set:
                    if other is not line and other.use > use:
                        other.use -= 1
        elif self._replace_pol == L1Cache.LFU:
                line.use += 1
